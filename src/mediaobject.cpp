...
void MediaObject::setSource(const MediaSource &source)
{
	...
	// Reload VLC with DSP callback handler
	// MediaObject::setSource("PhononDSP::effectCallback=xxxx") should be called just
	// before the MediaObject is created because VLC is reloaded
	if(source.url().toEncoded().contains("PhononDSP::effectCallback=")){
		// Unload the currently loaded context
		unloadMedia();
		m_player->stop();
		delete m_player;
		delete LibVLC::self;
		...
		// Set callback
		QList<QByteArray> vlc_args;
		qulonglong cb_ptr = source.url().toEncoded().mid(sizeof("PhononDSP::effectCallback")).toULongLong(0, 16);
		vlc_args << "--audio-filter=dsp";
		vlc_args << QString("--dsp-callback=%1").arg(cb_ptr, 0, 16).toAscii();

		// I also added an optionnal argument in LibVLC::init() which add given args
		LibVLC::init(vlc_args);
		m_player = new MediaPlayer(this);
		if (!m_player->libvlc_media_player())
			error() << "libVLC:" << LibVLC::errorMessage();
		// Player signals.
		...
		// Return since it is not an actual media source
		return;
	}
